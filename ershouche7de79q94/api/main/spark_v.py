...
def upload_csv():
    ...
    cursor.execute("SELECT * FROM usedcar")
    usedcar_column_info = cursor.fetchall()
    # 将数据写入 CSV 文件
    usedcar_path = os.path.join(parent_directory, "usedcar.csv")
    with open(usedcar_path, 'w', newline='', encoding='utf-8') as csvfile:
        writer = csv.writer(csvfile)
        # 写入表头
        writer.writerow([desc[0] for desc in cursor.description])
        # 写入数据行
        for row in usedcar_column_info:
            writer.writerow(row)
    # 上传映射文件
    usedcar_hdfs_csv_path = f'/input/usedcar.csv'
    usedcar_local_csv_path = os.path.join(parent_directory,"usedcar.csv")
    # 删除已有的数据
    if hadoop_client.status(usedcar_hdfs_csv_path, strict=False):
        hadoop_client.delete(usedcar_hdfs_csv_path, recursive=True)
    hadoop_client.upload(usedcar_hdfs_csv_path, usedcar_local_csv_path)
    # 上传group文件
...
#spark分析
@main_bp.route("/ershouche7de79q94/spark/analyze", methods=['GET'])
def spark_analyze():
