...
class Base_model(db.Model):
    ...
    def __Page(self, model, params, or_clauses):
        '''
        刷表专用
        http://ip:port/Ershouche7de79q94/${tableName}/page
        page 当前页
        pagesize 每页记录的长度
    ...
    def __GetByColumn(self, model, column, params):
        # 构造查询条件
        conditions = []
        if "refConditionQueryMethod" in params:
            refConditionColumn = params["refConditionColumn"]
            refConditionValue = params["refConditionValue"]
            if params["refConditionQueryMethod"]=='in':
                for value in refConditionValue.split(","):
                    conditions.append(getattr(model, refConditionColumn).in_(value))
            elif params["refConditionQueryMethod"]=='like':
                conditions.append(getattr(model, refConditionColumn).ilike(f'%{refConditionValue}%'))
            else:
                conditions.append(getattr(model, refConditionColumn) == refConditionValue)
            del params["refConditionQueryMethod"]
            del params["refConditionColumn"]
            del params["refConditionValue"]

        data1 = model.query.filter(*conditions).filter_by(**params).with_entities(column).all()
        data_list = []
        for i in data1:
    ...
    def __GroupByColumnName(self, model, columnName, params,between_params):
        query = db.session.query(getattr(model, columnName), func.count(getattr(model, columnName))).group_by(
            getattr(model, columnName))
        if len(between_params) == 0:
            query = query.filter_by(**params)
        else:
            date_filters = []
            for key, value in between_params.items():
                key = getattr(model, key.split(",")[0])
                date_filters.append(
                    and_(
                        key >= value.split(",")[0],
                        key <= value.split(",")[1]
            ...
            query = query.filter(and_(*date_filters)).filter_by(**params)
        try:
            if model.__tablename__ == 'orders' and hasattr(model, 'status'):
                query = query.filter(getattr(model, 'status').in_(['已支付', '已发货', '已完成']))
    ...
    def groupbycolumnname(self, model, columnName, params={},between_params={}):
        '''
        类别统计
        :param model:
        :param params:
        :return:
        '''
        return self.__GroupByColumnName(self, model, columnName, params,between_params)
        
    def __GetValueByxyColumnName(self,model,xColumnName,yColumnName,params,between_params):
        '''
        内部函数
        :param model:
        :param xColumnName:
        ...
        :return:
        '''
        func_params = params.get("func")
        func_entitiess = func.round(func.sum(getattr(model, yColumnName)),2)
        # 平均、最大、最小、总和、总数
        if func_params == "平均":
            func_entitiess = func.round(func.avg(getattr(model, yColumnName)),2)
        elif func_params == "最大":
            func_entitiess = func.max(getattr(model, yColumnName))
        elif func_params == "最小":
            func_entitiess = func.min(getattr(model, yColumnName))
        elif func_params == "总和":
            func_entitiess = func.count(getattr(model, yColumnName))
        try:
            del params["func"]
        except:
            pass
        query = db.session.query(getattr(model, xColumnName), func_entitiess).group_by(
            getattr(model, xColumnName))
        if len(between_params) == 0:
            query = query.filter_by(**params)
        else:
            date_filters = []
    ...
    def getvaluebyxycolumnname(self,model,xColumnName,yColumnName,params={},between_params={}):
        '''
        ...
        :return:
        '''
        return self.__GetValueByxyColumnName(self,model,xColumnName,yColumnName,params,between_params)
