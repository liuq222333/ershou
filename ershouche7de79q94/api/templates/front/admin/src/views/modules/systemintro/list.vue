...
<template>
	<div class="main-content systemintro-list-page">
		<div class="systemintro-page-head">
			<div class="systemintro-page-title-block">
				<h2 class="systemintro-page-title">系统简介管理</h2>
				<p class="systemintro-page-subtitle">维护平台简介信息与多图内容，保障首页展示一致性</p>
			</div>
			<div class="systemintro-page-metrics">
		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"borderRadius":"0","width":"100%","margin":"0"}' :inline="true" :model="searchForm">
				<el-row class="query-row" :style='{"padding":"0","boxShadow":"nnoe","borderColor":"#00000050","margin":"0","borderRadius":"0px","flexWrap":"wrap","borderWidth":"0","background":"none","display":"flex","width":"100%","position":"relative","borderStyle":"solid"}' >
					<div class="query-placeholder">简介数据默认按最新时间排序，可直接刷新、维护与删除内容</div>
				</el-row>
					...
					<el-table-column  :resizable='true' prop="picture1" width="200" label="图片1">
						<template slot-scope="scope">
							<div v-if="scope.row.picture1">
								<img class="intro-thumb" v-if="scope.row.picture1.substring(0,4)=='http'&&scope.row.picture1.split(',w').length>1" :src="scope.row.picture1" @click="imgPreView(scope.row.picture1)">
								<img class="intro-thumb" v-else-if="scope.row.picture1.substring(0,4)=='http'" :src="scope.row.picture1.split(',')[0]" @click="imgPreView(scope.row.picture1.split(',')[0])">
								<img class="intro-thumb" v-else :src="$base.url+scope.row.picture1.split(',')[0]" @click="imgPreView($base.url+scope.row.picture1.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					<el-table-column  :resizable='true' prop="picture2" width="200" label="图片2">
						<template slot-scope="scope">
							<div v-if="scope.row.picture2">
								<img class="intro-thumb" v-if="scope.row.picture2.substring(0,4)=='http'&&scope.row.picture2.split(',w').length>1" :src="scope.row.picture2" @click="imgPreView(scope.row.picture2)">
								<img class="intro-thumb" v-else-if="scope.row.picture2.substring(0,4)=='http'" :src="scope.row.picture2.split(',')[0]" @click="imgPreView(scope.row.picture2.split(',')[0])">
								<img class="intro-thumb" v-else :src="$base.url+scope.row.picture2.split(',')[0]" @click="imgPreView($base.url+scope.row.picture2.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					<el-table-column  :resizable='true' prop="picture3" width="200" label="图片3">
						<template slot-scope="scope">
							<div v-if="scope.row.picture3">
								<img class="intro-thumb" v-if="scope.row.picture3.substring(0,4)=='http'&&scope.row.picture3.split(',w').length>1" :src="scope.row.picture3" @click="imgPreView(scope.row.picture3)">
								<img class="intro-thumb" v-else-if="scope.row.picture3.substring(0,4)=='http'" :src="scope.row.picture3.split(',')[0]" @click="imgPreView(scope.row.picture3.split(',')[0])">
								<img class="intro-thumb" v-else :src="$base.url+scope.row.picture3.split(',')[0]" @click="imgPreView($base.url+scope.row.picture3.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
...
<style lang="scss" scoped>
	...
	.systemintro-page-subtitle {
		margin: 8px 0 0;
		color: var(--hw-sub);
		font-size: 14px;
	}

	.systemintro-page-metrics {
		display: flex;
		align-items: center;
		gap: 8px;
		flex-wrap: wrap;
