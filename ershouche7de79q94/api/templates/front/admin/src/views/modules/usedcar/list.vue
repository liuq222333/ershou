...
<template>
	<div class="main-content usedcar-list-page">
		<div class="usedcar-page-head">
			<div class="usedcar-page-title-block">
				<h2 class="usedcar-page-title">二手车管理</h2>
				<p class="usedcar-page-subtitle">统一管理车辆数据、价格信息与统计分析任务</p>
			</div>
			<div class="usedcar-page-metrics">
			...
			<el-form class="center-form-pv" :style='{"borderRadius":"0","width":"100%","margin":"0"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0","boxShadow":"nnoe","borderColor":"#00000050","margin":"0","borderRadius":"0px","flexWrap":"wrap","borderWidth":"0","background":"none","display":"flex","width":"100%","position":"relative","borderStyle":"solid"}' >
					<div class="query-field" :style='{"margin":"0 1% 20px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#333333","display":"inline-block","lineHeight":"41px","fontSize":"16px","fontWeight":"500","height":"41px"}' class="item-label">品牌</label>
						<el-input v-model="searchForm.brand" placeholder="品牌" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div class="query-field" :style='{"margin":"0 1% 20px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#333333","display":"inline-block","lineHeight":"41px","fontSize":"16px","fontWeight":"500","height":"41px"}' class="item-label">型号</label>
						<el-input v-model="searchForm.model1" placeholder="型号" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div class="query-field" :style='{"margin":"0 1% 20px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#333333","display":"inline-block","lineHeight":"41px","fontSize":"16px","fontWeight":"500","height":"41px"}' class="item-label">年份</label>
						<el-input v-model="searchForm.vehicleage" placeholder="年份" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"padding":"0","boxShadow":"none","margin":"0px 0 0px -4px","borderRadius":"8px","flexWrap":"wrap","background":"none","display":"flex","width":"calc(100% + 8px)","justifyContent":"0"}'>
					<el-button class="add" v-if="isAuth('usedcar','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia17" :style='{"margin":"0 0px","fontSize":"16px","color":"#883434","display":"none","height":"auto"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('usedcar','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 0px","fontSize":"16px","color":"#883434","display":"none","height":"auto"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('usedcar','爬取数据')" type="success" :loading="spiderLoading" @click="spider()">
						<span v-if="!spiderLoading" class="icon iconfont icon-shuju17" :style='{"margin":"0 0px","fontSize":"16px","color":"#883434","display":"none","height":"auto"}'></span>
						{{ spiderLoading ? '爬取中...' : '爬取数据' }}
					</el-button>
					...
					<el-button class="btn18" v-if="isAuth('usedcar','品牌统计')" type="success" @click="chartDialogShow1">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"16px","color":"#883434","display":"none","height":"auto"}'></span>
						品牌统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('usedcar','型号统计')" type="success" @click="chartDialogShow2">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"16px","color":"#883434","display":"none","height":"auto"}'></span>
						型号统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('usedcar','城市统计')" type="success" @click="chartDialogShow3">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"16px","color":"#883434","display":"none","height":"auto"}'></span>
						城市统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('usedcar','年份统计')" type="success" @click="chartDialogShow4">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"16px","color":"#883434","display":"none","height":"auto"}'></span>
						年份统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('usedcar','车辆现价')" type="success" @click="chartDialogShow5">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"16px","color":"#883434","display":"none","height":"auto"}'></span>
						车辆现价
					</el-button>
					<el-button class="btn18" v-if="isAuth('usedcar','里程统计')" type="success" @click="chartDialogShow6">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"16px","color":"#883434","display":"none","height":"auto"}'></span>
						里程统计
					</el-button>
				</el-row>
			...
			<div class="table-shell" :style='{"width":"100%","padding":"0","margin":"20px 0 0 0","borderRadius":"8px","background":"#fff","borderWidth":"0"}'>
				<el-table class="tables"
					...
					<el-table-column  :resizable='true' prop="imgurl" width="200" label="图片">
						<template slot-scope="scope">
							<div v-if="scope.row.imgurl">
								<img class="car-thumb" v-if="scope.row.imgurl.substring(0,4)=='http'&&scope.row.imgurl.split(',w').length>1" :src="scope.row.imgurl" @click="imgPreView(scope.row.imgurl)">
								<img class="car-thumb" v-else-if="scope.row.imgurl.substring(0,4)=='http'" :src="scope.row.imgurl.split(',')[0]" @click="imgPreView(scope.row.imgurl.split(',')[0])">
								<img class="car-thumb" v-else :src="$base.url+scope.row.imgurl.split(',')[0]" @click="imgPreView($base.url+scope.row.imgurl.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('usedcar','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								详情
							</el-button>
							<el-button class="edit" v-if=" isAuth('usedcar','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="view" v-if="isAuth('usedcar','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								查看评论
							</el-button>
							...
							<el-button class="del" v-if="isAuth('usedcar','删除')" type="primary" @click="deleteHandler(scope.row.id)">
		...
		<el-dialog
			...
			width="800">
			<div id="model1Chart2" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
		...
		<el-dialog
			...
			width="800">
			<div id="cityChart3" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
		...
		<el-dialog
			...
			width="800">
			<div id="vehicleageChart4" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
		...
		<el-dialog
			...
			width="800">
			<div id="discountpriceChart5" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
		...
		<el-dialog
			...
			width="800">
			<div id="kilometerChart6" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
	...
	export default {
		data() {
			return {
				...
				gauge: {"tooltip":{"backgroundColor":"#123","textStyle":{"color":"#fff"}},"backgroundColor":"transparent","color":["#883434","#ae9e35","#333333","#e18d46","#f05a08","#D2462E","#4495ac","#9a60b4","#ea7ccc"],"title":{"top":"top","left":"left","textStyle":{"fontSize":14,"lineHeight":24,"color":"#333","fontWeight":600}},"series":{"pointer":{"offsetCenter":[0,"10%"],"icon":"path://M2.9,0.7L2.9,0.7c1.4,0,2.6,1.2,2.6,2.6v115c0,1.4-1.2,2.6-2.6,2.6l0,0c-1.4,0-2.6-1.2-2.6-2.6V3.3C0.3,1.9,1.4,0.7,2.9,0.7z","width":8,"length":"80%"},"axisLine":{"lineStyle":{"shadowOffsetX":0,"shadowOffsetY":0,"opacity":0.5,"shadowBlur":1,"shadowColor":"#000"},"roundCap":true},"anchor":{"show":true,"itemStyle":{"color":"inherit"},"size":18,"showAbove":true},"emphasis":{"disabled":false},"progress":{"show":true,"roundCap":true,"overlap":true},"splitNumber":25,"detail":{"formatter":"{value}","backgroundColor":"inherit","color":"#fff","borderRadius":3,"width":20,"fontSize":12,"height":10},"title":{"fontSize":14},"animation":true}},
		...
		methods: {
			...
			chartDialog2() {
				this.$nextTick(()=>{
					var model1Chart2 = echarts.init(document.getElementById("model1Chart2"),'macarons');
					let params = {}
					this.$http({
			...
			chartDialog3() {
				this.$nextTick(()=>{
					var cityChart3 = echarts.init(document.getElementById("cityChart3"),'macarons');
					let params = {}
					this.$http({
			...
			chartDialog4() {
				this.$nextTick(()=>{
					var vehicleageChart4 = echarts.init(document.getElementById("vehicleageChart4"),'macarons');
					let params = {}
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							vehicleageChart4.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								vehicleageChart4.resize();
							};
						}
					})
				})
			...
			chartDialog5() {
				this.$nextTick(()=>{
					var discountpriceChart5 = echarts.init(document.getElementById("discountpriceChart5"),'macarons');
					let params = {}
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							discountpriceChart5.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								discountpriceChart5.resize();
							};
						}
					})
				})
			...
			chartDialog6() {
				this.$nextTick(()=>{
					var kilometerChart6 = echarts.init(document.getElementById("kilometerChart6"),'macarons');
					let params = {}
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							kilometerChart6.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								kilometerChart6.resize();
							};
						}
					})
				})
			...
			getDataList() {
				...
				if(this.searchForm.model1!='' && this.searchForm.model1!=undefined){
					params['model1'] = '%' + this.searchForm.model1 + '%'
				}
				if(this.searchForm.vehicleage!='' && this.searchForm.vehicleage!=undefined){
					params['vehicleage'] = '%' + this.searchForm.vehicleage + '%'
				}
			...
			disscussListHandler(id,type) {
				this.$router.push({path:'/discussusedcar',query:{refid:id}});
			},
			// 删除
			...
			spider() {
				...
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.$message.error(data.msg || "爬取失败");
					}
				}).catch(() => {
					this.spiderLoading = false;
					this.$message.error("爬取请求超时或失败");
				});
			},
...
<style lang="scss" scoped>
	.usedcar-list-page {
		...
		background:
			radial-gradient(circle at 0% 0%, rgba(207, 10, 44, 0.06) 0%, transparent 36%),
	...
	.usedcar-page-subtitle {
		margin: 8px 0 0;
		color: var(--hw-sub);
		font-size: 14px;
	}

	.usedcar-page-metrics {
		display: flex;
		align-items: center;
		gap: 8px;
	}
	...
	.chartDialog ::v-deep .el-dialog__header {
		border-bottom: 1px solid #ecf0f6;
	}

	.chartDialog ::v-deep .el-dialog__title {
		color: #2f3440;
		font-weight: 700;
	}
