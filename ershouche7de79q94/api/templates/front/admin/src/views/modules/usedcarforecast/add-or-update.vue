...
<template>
	<div class="addEdit-block">
		...
		>
			...
			<el-form-item class="input" v-if="type!='info'"  label="预测价" prop="discountprice" style="display:none">
				<el-input-number v-model="ruleForm.discountprice" placeholder="预测价" :disabled="ro.discountprice"></el-input-number>
			</el-form-item>
			<el-form-item v-else class="input" label="预测价" prop="discountprice" >
				<el-input v-model="ruleForm.discountprice" placeholder="预测价" readonly></el-input>
			</el-form-item>
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `usedcarforecast/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.usedcarforecastCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
