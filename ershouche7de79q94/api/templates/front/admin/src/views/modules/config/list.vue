...
<template>
	<div class="main-content config-list-page">
		<div class="config-page-head">
			<div class="config-page-title-block">
				<h2 class="config-page-title">{{ configPageTitle }}</h2>
				<p class="config-page-subtitle">{{ configPageSubtitle }}</p>
			</div>
		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"borderRadius":"0","width":"100%","margin":"0"}' :inline="true" :model="searchForm">
				<el-row class="query-row" :style='{"padding":"0","boxShadow":"nnoe","borderColor":"#00000050","margin":"0 0 8px","borderRadius":"0px","flexWrap":"wrap","borderWidth":"0","background":"none","display":"flex","width":"100%","position":"relative","borderStyle":"solid"}'>
					<div class="query-placeholder">可维护配置项名称与图片资源，当前支持预览、编辑与顺序管理</div>
				</el-row>
					...
					<el-table-column  :resizable='true' prop="value" width="200" label="图片">
						<template slot-scope="scope">
							<div v-if="scope.row.value">
								<img class="config-thumb" v-if="scope.row.value.substring(0,4)=='http'&&scope.row.value.split(',w').length>1" :src="scope.row.value" @click="imgPreView(scope.row.value)">
								<img class="config-thumb" v-else-if="scope.row.value.substring(0,4)=='http'" :src="scope.row.value.split(',')[0]" @click="imgPreView(scope.row.value.split(',')[0])">
								<img class="config-thumb" v-else :src="$base.url+scope.row.value.split(',')[0]" @click="imgPreView($base.url+scope.row.value.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
...
<script>
	...
	export default {
		...
		computed: {
			...
			configPageTitle() {
				return String(this.configStatus) === '1' ? '轮播图管理' : '配置管理'
			},
			configPageSubtitle() {
				return String(this.configStatus) === '1'
					? '统一维护轮播图资源与展示顺序，确保首页视觉质量'
					: '统一维护系统配置项与图像资源'
			},
		},
		components: {
