...
<template>
	<div class="main-content user-list-page">
		<div class="user-page-head">
			<div class="user-page-title-block">
				<h2 class="user-page-title">用户管理</h2>
				<p class="user-page-subtitle">统一管理账号、实名信息与联系方式</p>
			</div>
			<div class="user-page-metrics">
			...
			<el-form class="center-form-pv" :style='{"borderRadius":"0","width":"100%","margin":"0"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0","boxShadow":"nnoe","borderColor":"#00000050","margin":"0","borderRadius":"0px","flexWrap":"wrap","borderWidth":"0","background":"none","display":"flex","width":"100%","position":"relative","borderStyle":"solid"}' >
					<div class="query-field" :style='{"margin":"0 1% 20px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#333333","display":"inline-block","lineHeight":"41px","fontSize":"16px","fontWeight":"500","height":"41px"}' class="item-label">账号</label>
						<el-input v-model="searchForm.zhanghao" placeholder="账号" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div class="query-field" :style='{"margin":"0 1% 20px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#333333","display":"inline-block","lineHeight":"41px","fontSize":"16px","fontWeight":"500","height":"41px"}' class="item-label">姓名</label>
						<el-input v-model="searchForm.xingming" placeholder="姓名" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div class="query-field select" :style='{"margin":"0 1% 20px 0","display":"flex"}' label="性别" prop="xingbie">
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#333333","display":"inline-block","lineHeight":"41px","fontSize":"16px","fontWeight":"500","height":"41px"}' class="item-label">性别</label>
					...
					<el-table-column  :resizable='true' prop="touxiang" width="200" label="头像">
						<template slot-scope="scope">
							<div v-if="scope.row.touxiang">
								<img class="avatar-thumb" v-if="scope.row.touxiang.substring(0,4)=='http'&&scope.row.touxiang.split(',w').length>1" :src="scope.row.touxiang" @click="imgPreView(scope.row.touxiang)">
								<img class="avatar-thumb" v-else-if="scope.row.touxiang.substring(0,4)=='http'" :src="scope.row.touxiang.split(',')[0]" @click="imgPreView(scope.row.touxiang.split(',')[0])">
								<img class="avatar-thumb" v-else :src="$base.url+scope.row.touxiang.split(',')[0]" @click="imgPreView($base.url+scope.row.touxiang.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
